...
class CheckOutUtilityReading(models.Model):
    ...
    check_out_utility_reading_id = models.AutoField(primary_key=True)
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="check_out_utility_readings_created"
    )
    ...
    def __str__(self):
        return f"{self.utility_type} reading for CheckOut {self.check_out_id}"
    ...
    def create(
        ...
        self.save()
        return self.check_out_utility_reading_id

    @staticmethod
    def get_all_for_check_out(check_out_id: int):
        return CheckOutUtilityReading.objects.filter(
            check_out_id=check_out_id, is_active=True
        ).values(
            "check_out_utility_reading_id", "utility_type", "meter_no", "reading_value",
    ...
    def update(
        ...
        try:
            reading = CheckOutUtilityReading.objects.get(check_out_utility_reading_id=check_out_utility_reading_id)
        except CheckOutUtilityReading.DoesNotExist:
            raise ValueError(f"Invalid Check-Out Utility Reading Id: {check_out_utility_reading_id}")
        ...
        reading.save()
        return reading.check_out_utility_reading_id

    @staticmethod
    def delete(check_out_utility_reading_id: int) -> None:
        CheckOutUtilityReading.objects.filter(
            check_out_utility_reading_id=check_out_utility_reading_id
