...
class CheckInInspectionItem(models.Model):
    CATEGORY_CHOICES = [
    ...
    check_in_inspection_item_id = models.AutoField(primary_key=True)
    ...
    assigned_to = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="check_in_inspection_items_assigned"
    )
    target_date = models.DateField(null=True, blank=True)
    photo = models.FileField(upload_to="checkin_checkout/check_in_inspection_items/", max_length=500, null=True, blank=True)
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="check_in_inspection_items_created"
    )
    ...
    def __str__(self):
        return f"{self.item_name} ({self.category}) for CheckIn {self.check_in_id}"
    ...
    def create(
        ...
        self.save()
        return self.check_in_inspection_item_id

    @staticmethod
    def get_all_for_check_in(check_in_id: int):
        return CheckInInspectionItem.objects.filter(
            check_in_id=check_in_id, is_active=True
        ).values(
            "check_in_inspection_item_id", "category", "item_name", "inspection_status",
            "severity", "repair_status", "item_approval_status", "assigned_to_id", "assigned_to__name",
            "target_date", "photo", "remarks", "created_at",
        )

    @staticmethod
    def update(
        check_in_inspection_item_id: int,
        category: str = None,
        item_name: str = None,
        ...
        try:
            item = CheckInInspectionItem.objects.get(check_in_inspection_item_id=check_in_inspection_item_id)
        except CheckInInspectionItem.DoesNotExist:
            raise ValueError(f"Invalid Check-In Inspection Item Id: {check_in_inspection_item_id}")
        ...
        item.save()
        return item.check_in_inspection_item_id

    @staticmethod
    def delete(check_in_inspection_item_id: int) -> None:
        CheckInInspectionItem.objects.filter(
            check_in_inspection_item_id=check_in_inspection_item_id
