...
class OwnerUtils:
    def __init__(self, entity: str, columns_required: list = []) -> None:
        ...
        owner_map = {
            'owner_id': 'ownerId',
            'owner_id__phone_number': 'phoneNumber',
            'owner_id__email': 'email',
            'name': 'name',
            'dob': 'dob',
        ...
        self.mapped_columns_name = owner_map if entity == 'owner' else {}
    ...
    def reverse_mapper(fields: list[str]) -> dict[str, str]:
        combined_map = {}
        combined_map.update(OwnerUtils('owner').mapped_columns_name)
