...
class MarketingEmployeeView:
    ...
    def get_all_employee_extract(self, params: GetAll):
        ...
        employee_list = MarketingEmployee.get_all(
            sort_by=sort_by_db,
        ...
        if MarketingManager.objects.filter(manager_id=params.user_id).exists():
            employee_list = [row for row in employee_list if row.get('manager_ref_id') == params.user_id]
        elif MarketingEmployee.objects.filter(employee_id=params.user_id).exists():
