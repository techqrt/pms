...
class ITManagerView:
    def __init__(self):
        self.manager_create = "IT manager added successfully"
        self.manager_update = "IT manager updated successfully"
        self.manager_delete = "IT manager deleted successfully"
    ...
    def create_manager_extract(self, params: ITManagerCreateRequest):
        with transaction.atomic():
            ...
            manager_id = obj.create(
                ...
                projects_managed=params.projects_managed,
                systems_overseen=params.systems_overseen,
    ...
    @Common().exception_handler
    def update_manager_extract(self, params: ITManagerUpdateRequest):
        with transaction.atomic():
            if params.user_id != params.manager_id:
                raise ValueError("Not allowed to access this resource")
            manager_data = ITManager.get(manager_id=params.manager_id)
    ...
    def delete_manager_extract(self, params):
        with transaction.atomic():
            ...
            if manager_data is None:
                raise ValueError(self.data_no_match)
            ITManager.remove(manager_id=params.manager_id)
        return Response(status=status.HTTP_200_OK, data=Utils.success_response_data(message=self.manager_delete))

    @Common(response_handler=ITManagerResponseGetSerializer).exception_handler
    def get_manager_extract(self, params):
        with transaction.atomic():
            ...
            if manager_data is None:
                raise ValueError(self.data_no_match)
            utils = ITUtils(entity='manager', columns_required=[
    ...
    @Common(response_handler=ITManagerResponseGetAllSerializer).exception_handler
    def get_all_manager_extract(self, params: GetAll):
        ...
        manager_list = ITManager.get_all(
        ...
        if ITManager.objects.filter(manager_id=params.user_id).exists():
            ...
            employee_manager_ref_id = ITEmployee.objects.filter(
