...
class LegalManagerView:
    def __init__(self):
        self.manager_create = "Legal manager added successfully"
        self.manager_update = "Legal manager updated successfully"
        self.manager_delete = "Legal manager deleted successfully"
    ...
    def create_manager_extract(self, params: LegalManagerCreateRequest):
        with transaction.atomic():
            ...
            manager_id = obj.create(
                ...
                total_cases_handled=params.total_cases_handled,
                open_cases=params.open_cases,
                closed_cases=params.closed_cases,
    ...
    @Common().exception_handler
    def update_manager_extract(self, params: LegalManagerUpdateRequest):
        with transaction.atomic():
            if params.user_id != params.manager_id:
                raise ValueError("Not allowed to access this resource")
            manager_data = LegalManager.get(manager_id=params.manager_id)
    ...
    def delete_manager_extract(self, params):
        with transaction.atomic():
            ...
            if manager_data is None:
                raise ValueError(self.data_no_match)
            LegalManager.remove(manager_id=params.manager_id)
        return Response(status=status.HTTP_200_OK, data=Utils.success_response_data(message=self.manager_delete))

    @Common(response_handler=LegalManagerResponseGetSerializer).exception_handler
    def get_manager_extract(self, params):
        with transaction.atomic():
            ...
            if manager_data is None:
                raise ValueError(self.data_no_match)
            utils = LegalUtils(entity='manager', columns_required=[
    ...
    @Common(response_handler=LegalManagerResponseGetAllSerializer).exception_handler
    def get_all_manager_extract(self, params: GetAll):
        ...
        manager_list = LegalManager.get_all(
        ...
        # unrestricted view.
        if LegalManager.objects.filter(manager_id=params.user_id).exists():
            manager_list = [row for row in manager_list if row.get('manager_id') == params.user_id]
        else:
            employee_manager_ref_id = LegalEmployee.objects.filter(
