...
class MaintenanceUtils:
    ...
    def reverse_mapper(fields: list[str]) -> dict[str, str]:
        combined_map = {}
        combined_map.update(MaintenanceUtils('manager').mapped_columns_name)
        combined_map.update(MaintenanceUtils('employee').mapped_columns_name)
        combined_map.update(MaintenanceUtils('technician').mapped_columns_name)
