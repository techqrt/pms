...
class CollectionEmployeeView:
    def __init__(self):
        self.employee_create = "Collection employee added successfully"
        self.employee_update = "Collection employee updated successfully"
        self.employee_delete = "Collection employee deleted successfully"
    ...
    def create_employee_extract(self, params: CollectionEmployeeCreateRequest):
        with transaction.atomic():
            _ = User.get(user_id=params.employee_id)

            obj = CollectionEmployee()
            employee_id = obj.create(
                ...
                collections_made=params.collections_made,
                overdue_accounts_handled=params.overdue_accounts_handled,
    ...
    @Common().exception_handler
    def update_employee_extract(self, params: CollectionEmployeeUpdateRequest):
        with transaction.atomic():
            if params.user_id != params.employee_id:
                raise ValueError("Not allowed to access this resource")
            employee_data = CollectionEmployee.get(
                employee_id=params.employee_id)
            if employee_data is None:
                raise ValueError(self.data_no_match)

            CollectionEmployee.update(
                employee_id=params.employee_id,
                name=params.name,
                dob=params.dob,
                designation=params.designation,
    ...
    def delete_employee_extract(self, params):
        with transaction.atomic():
            ...
            if employee_data is None:
                raise ValueError(self.data_no_match)
            CollectionEmployee.remove(employee_id=params.employee_id)
        return Response(status=status.HTTP_200_OK, data=Utils.success_response_data(message=self.employee_delete))

    @Common(response_handler=CollectionEmployeeResponseGetSerializer).exception_handler
    def get_employee_extract(self, params):
        with transaction.atomic():
            ...
            if employee_data is None:
                raise ValueError(self.data_no_match)
            utils = CollectionUtils(entity='employee', columns_required=[
    ...
    @Common(response_handler=CollectionEmployeeResponseGetAllSerializer).exception_handler
    def get_all_employee_extract(self, params: GetAll):
        ...
        employee_list = CollectionEmployee.get_all(
            sort_by=sort_by_db,
        ...
        if CollectionManager.objects.filter(manager_id=params.user_id).exists():
            employee_list = [row for row in employee_list if row.get('manager_ref_id') == params.user_id]
        elif CollectionEmployee.objects.filter(employee_id=params.user_id).exists():
